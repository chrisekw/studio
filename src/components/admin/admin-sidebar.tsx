    ...
    <Sidebar collapsible="icon" variant="sidebar" side="left">
      <SidebarHeader>
        ...
        >
          ...
          <span className={state === 'collapsed' ? 'hidden': 'font-headline text-lg font-bold'}>oPilot Admin</span>
        </Link>
      </SidebarHeader>
      <SidebarContent>
        <AdminSidebarNav />
      ...
      <SidebarFooter>
        <SidebarSeparator />
         <div className="p-2 group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:justify-center">
            <Button asChild variant="ghost" className="w-full justify-start p-2 h-auto text-muted-foreground hover:text-foreground">
                <Link href="/dashboard">
                    <ArrowLeft />
                    <span className="group-data-[collapsible=icon]:hidden ml-2">Back to App</span>
                </Link>
            </Button>
        </div>
        {user && (
